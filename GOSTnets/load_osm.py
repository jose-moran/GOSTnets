...
from shapely import speedups, ops
...
class OSM_to_network(object):
    ...
    def __init__(self, osmFile, includeFerries=False):
        ...
        self.roads_raw = self.fetch_roads_and_ferries(
            osmFile) if includeFerries else self.fetch_roads(osmFile)
    ...
    def generateRoadsGDF(self, in_df=None, outFile='', verbose=False):
        ...
        nodes = gpd.GeoDataFrame(roads.apply(
            lambda x: get_nodes(x), axis=1).apply(pd.Series))
        nodes.columns = ['u', 'v']
    ...
    def filterRoads(self, acceptedRoads=['primary', 'primary_link', 'secondary', 'secondary_link', 'motorway', 'motorway_link', 'trunk', 'trunk_link']):
        ...
        self.roads_raw = self.roads_raw.loc[self.roads_raw.infra_type.isin(
            acceptedRoads)]
    ...
    def fetch_roads(self, data_path):
        ...
        if data_path.split('.')[-1] == 'pbf':
            ...
            for feature in sql_lyr:
                if feature.GetField("highway") is not None:
                    ...
                    if feature.GetField('other_tags'):
                        ...
                        elements = other_tags.split(',')
                        elements = [
                            element for element in elements if '=>' in element]
            ...
            if len(roads) > 0:
                road_gdf = gpd.GeoDataFrame(roads, columns=[
                                            'osm_id', 'infra_type', 'one_way', 'geometry'], crs={'init': 'epsg:4326'})
                return road_gdf
        ...
        if data_path.split('.')[-1] == 'pbf':
            ...
            if len(roads) > 0:
                road_gdf = gpd.GeoDataFrame(
                    roads, columns=['osm_id', 'infra_type', 'geometry'], crs={'init': 'epsg:4326'})
                return road_gdf
    ...
    def line_length(self, line, ellipsoid='WGS-84'):
        ...
        return sum(
            distance.geodesic(tuple(reversed(a)), tuple(
                reversed(b)), ellipsoid=ellipsoid).km
    ...
    def get_all_intersections(self, shape_input, idx_osm=None, unique_id='osm_id', verboseness=False):
        ...
        for idx, row in shape_input.iterrows():
            ...
            count += 1
            intersections = shape_input.iloc[list(
                idx_osm.intersection(line.bounds))]
            intersections = dict(
            ...
            hits = [n.object for n in idx_inters.intersection(
                line.bounds, objects=True)]

            if len(hits) != 0:
                try:
                    out = shapely.ops.split(line, MultiPoint(hits))
                    new_lines.append([{'geometry': LineString(
                        x), 'osm_id': key1, 'infra_type': infra_type, 'one_way': one_way} for x in out.geoms])
                except:
                    pass
            else:
